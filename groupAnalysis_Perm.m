    ...
    if subject_nr ~= 6
        for poifile = 1:3
            ...
            elseif poifile == 2
                ...
                pooledApc_Poi2.permAcc(1:1000,subject_nr) = Apc;
                pooledSpc_Poi2.permAcc(1:1000,subject_nr) = Spc;

                pooledApc_Poi2.obsAcc(1,subject_nr) = mean(svmOutObs.av);
                pooledSpc_Poi2.obsAcc(1,subject_nr) = mean(svmOutObs.pc);
            % Auditory cortex
            else
                patch = 1;
                ...
                filename = ['Results/',folder,'/sub-0',int2str(subject_nr),'_MainAnalysis_CollapseHem_Patch',num2str(patch),'_POI',num2str(poifile)];
                load(filename);
                pooledApc_Poi1.permAcc(1:1000,subject_nr) = Apc;
                pooledSpc_Poi1.permAcc(1:1000,subject_nr) = Spc;
                % For the observed values
                pooledApc_Poi1.obsAcc(1,subject_nr) = mean(svmOutObs.av);
                pooledSpc_Poi1.obsAcc(1,subject_nr) = mean(svmOutObs.pc);
...
save('Results/pooledPermResults','pooledApc_Poi3','pooledSpc_Poi3','pooledApc_Poi2','pooledSpc_Poi2','pooledApc_Poi1','pooledSpc_Poi1');
...
group_pPerm_Apc.V1 = length(find(meanApc.V1 >= meanObsApc.V1)) ./ 1000;
group_pPerm_Apc.V2 = length(find(meanApc.V2 >= meanObsApc.V2)) ./ 1000;
...
group_pPerm_Spc.V1 = length(find(meanSpc.V1 >= meanObsSpc.V1)) ./ 1000;
group_pPerm_Spc.V2 = length(find(meanSpc.V2 >= meanObsSpc.V2)) ./ 1000;

save('Results/groupResults_Perm','meanSpc','meanApc','meanObsApc','meanObsSpc','group_pPerm_Apc','group_pPerm_Spc');
