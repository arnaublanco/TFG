...
%this runs the single SVM for all subjects computing the
%non-biased confusion matrices
...
for subject = 1:length(subjects)
    for POIfile_ind = 1:3    
        if POIfile_ind == 3
            for patch = 1:16
                run_single_sub_CollapseHem_get_unbiased_conf_mats(subjects{subject},patch,CondClass,POIfile_ind);
            end
        else
            patch = 1;
            run_single_sub_CollapseHem_get_unbiased_conf_mats(subjects{subject},patch,CondClass,POIfile_ind);
